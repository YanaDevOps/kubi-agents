...
function safeStructured(value, depth = 0) {
  if (depth > 6 || value == null || ['boolean', 'number'].includes(typeof value)) return value ?? null;
  if (typeof value === 'string') return safeText(value, 4096);
  if (Array.isArray(value)) return value.slice(0, 200).map((entry) => safeStructured(entry, depth + 1));
  if (typeof value !== 'object') return null;
  const result = {};
  for (const [key, entry] of Object.entries(value).slice(0, 200)) {
    const safeKey = text(key, 256);
    if (!safeKey || /token|password|authorization|credential|private.?key|api.?key/i.test(safeKey)) continue;
    result[safeKey] = safeStructured(entry, depth + 1);
  }
  return result;
}
...
function normalize(event) {
  ...
  const summary = safeText(event.summary || event.message || reason);
  const title = safeText(event.title || [resource.kind, resource.name, reason].filter(Boolean).join(' '), 1024);
  const row = { sourceId: text(event.sourceId), source: text(event.source || event.sourceId), namespace: text(event.namespace ?? resource.namespace),
    severity: text(event.severity || 'info', 32), category: text(event.category || 'event', 64),
    reason, title, summary, message: summary, resource,
    ...
    firstObservedAt: iso(observed), lastObservedAt: iso(observed), firstSeenAt: iso(observed), lastSeenAt: iso(observed), count: 1,
    before: safeStructured(event.before), after: safeStructured(event.after),
    related: safeStructured(event.related), logs: safeStructured(event.logs) };
...
function append(key, event) {
  ...
  const ids = transaction(db, () => normalized.map(({ row, dedup, requestedId, cumulative }) => {
    ...
    if (previous) {
      ...
      row.firstSeenAt = previous.firstSeenAt || previous.firstObservedAt || row.firstSeenAt;
      row.lastSeenAt = row.lastObservedAt;
    ...
    row.severity, row.category, [row.title, row.summary, row.reason, ...Object.values(row.resource)].join(' ').toLowerCase(), JSON.stringify(row));
    return row.id;
  }));
  enforce();
...
const stateFields = new Set(['uid', 'id', 'kind', 'name', 'namespace', 'apiVersion', 'resourceVersion',
  ...
  'conditions', 'containerStatuses', 'initContainerStatuses', 'ephemeralContainerStatuses',
  'currentRevision', 'updateRevision', 'nodeInfo', 'kubeletVersion', 'unschedulable', 'suspend',
  'template', 'containers', 'initContainers', 'image', 'state', 'lastState', 'running', 'waiting', 'terminated', 'ready',
  'exitCode', 'finishedAt', 'startedAt', 'lastTransitionTime', 'involvedObject', 'regarding', 'note', 'series', 'lastObservedTime',
  'deprecatedCount', 'deprecatedLastTimestamp', 'eventTime',
...
function sanitizeState(state) {
  let nodes = 0;
  function clean(value, depth, mapDepth = 0) {
    ...
    for (const [key, entry] of Object.entries(value)) {
      if (/token|secret|password|authorization|credential|kubeconfig|^data$|^annotations$|^managedFields$/i.test(key)) continue;
      if (mapDepth > 0 || stateFields.has(key)) result[text(key, 256)] = clean(entry, depth + 1, Math.max(0, mapDepth - 1));
    }
    return result;
  }
  const result = {};
