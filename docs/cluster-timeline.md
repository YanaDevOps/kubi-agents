...
events. Later bounded polls record resource creation/deletion, meaningful
Kubernetes warnings and normal events, pod/container failures and recoveries,
node pressure/readiness changes, workload and storage transitions, and
supported delivery/backup status changes. Routine informational events are
filtered. A source returning after a permission or network gap establishes a
